...
summaryRESSTAT <-
  function(t_frame,
    ...
    if (sum == T) {
      m_frame$class = gsub("(^[a-zA-Z0-9-]+)([^a-zA-Z0-9-].*)", "\\1", m_frame$variable)
    ...
    if(sum==F){
      ...
      t_frame=t_frame[,c(colnames(t_frame)[reorder_wide],colnames(t_frame)[2:(ncol(t_frame)-2)])]
    }
...
summaryRES_PACK_2<-function(dfc_obj, digitos=2, varTOcast, lipidclassColName=NULL, estat="media_se",my_comparisons,salto=1.1){
  ...
  if (ncol(p_y.position)>2) { #añadido para evitar error en comparaciones de solo dos grupos 
