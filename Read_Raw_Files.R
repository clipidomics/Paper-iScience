...
FileNames<-unique(data.metadata$FileName);FileNames
CellIDs<-unique(data.metadata$CellID);CellIDs
CellTypes<-unique(data.metadata$CellType);CellTypes
ColumnRawNames<-unique(colnames(data.raw));ColumnRawNames
...
data.seurat<-list()
min.cells.sel<-0 ## No se puede hacer de otro modo
min.features.sel<-0  ## No se puede hacer por partes 

#No esta claro que esto funcione!!!
...
#data.seurat.combo_chunk_10000<-data.seurat.combo
dim(data.seurat.combo)
...
#pbmc
###Save Seurat object
saveRDS(data.seurat.combo, file =   paste0( directory,"/output/hepatocyte_chow_full_no_filtering.rds"))

###Load Seurat object
#infoRDS(paste0( directory,"/output/hepatocyte_chow_full.rds"))
data.seurat<-readRDS(paste0( directory,"/output/hepatocyte_chow_full.rds"))
#identical(data.seurat.combo,data.seurat)
